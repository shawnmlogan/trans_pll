...
system(awk_comamnd_0);
system(awk_comamnd_1);
lost_lock_time_sec_string = system("cat ./.gnu_number_formatted");
system("rm ./.gnu_number ./.gnu_number_formatted");
...
set output sprintf("gnuplot_vcontrol_voltage_vs_time_%s.png",timestamp);
...
unset xrange;
unset yrange;
stats input_filename u 1:(column(column_num)) nooutput;
...
yrange_min = y_min/y_scale_factor;
yrange_max = y_max/y_scale_factor;
if (abs(yrange_min) < 1e-12) {
	yrange_min = 0.0;
} else {
...
if (num_lines > 10000) {
	...
	if (lock_time_sec > 0.0) {
		if (lost_lock_time_sec > 0.0) {
			plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) every skip_values title '' with lines linecolor rgb "red", $lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black", $lost_lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black";;
			} else {
			plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) every skip_values title '' with lines linecolor rgb "red", $lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black";
			}
	} else {
	plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) every skip_values title '' with lines linecolor rgb "red";
	}
} else {
	if (lock_time_sec > 0.0) {
		if (lost_lock_time_sec > 0.0) {
			plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) title '' with lines linecolor rgb "red", $lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black", $lost_lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black";
			} else {
			plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) title '' with lines linecolor rgb "red", $lock_time_vline using 1:2 title '' w impulse dt 8 linecolor rgb "black";
			}
	} else {
	plot input_filename u ($1/x_scale_factor):(column(column_num)/y_scale_factor) title '' with lines linecolor rgb "red";
	}
}
