...
overlap = 0.0;
nfft = length(phasein_ui_clipped)/window_divider;
fs = nom_freq;
conf = 0.95;
range ={"onesided"};
...
if (compute_phasein_ui_clipped_psd_flag == 1)
	...
	if (compute_phaseo_delayed_ui_clipped_psd_flag == 1)
		...
		print(plotname);
		psd_filename = sprintf("psd_osc_tie_vs_offset_window_divider_%.0f_overlap_%.0f_%s.csv",window_divider,100*overlap,timestamp);
		fp_psd = fopen(psd_filename,"w");
		fprintf(fp_psd,"Frequency (Hz),phasei PSD (UI^2/Hz),phaseo_delayed PSD (UI^2/Hz)\n");
		for i = 1:length(freq)
			fprintf(fp_psd,"%1.12e,%1.12e,%1.12e\n",freq(i),spectra(i),spectra_o(i));
		endfor
		fclose(fp_psd);
