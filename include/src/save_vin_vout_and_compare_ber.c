...
static long int last_cycle_check, number_of_error_free_samples;
	...
	if ((number_of_output_clock_cycles > number_of_output_samples) && (lock_detect_flag == 1) && (*perror_free_flag == 0))
			...
			if (sum == 0)
				...
				*pdelay = i;
				if (*pdelay == 0)
      			...
      			"Error free at time = %s, delay of %ld (± 0.5) clock cycle between input and output data.\n",
      			add_units(time,6,"s",value_string[0]),*pdelay + 1);
      			}
      		else
      			{
      			snprintf(plog_string,LOGFILE_LINELENGTH,
      			"Error free at time = %s, delay of %ld (± 0.5) clock cycles between input and output data.\n",
      			add_units(time,6,"s",value_string[0]),*pdelay + 1);
					}      			
		...
		if ((number_of_output_clock_cycles > number_of_output_samples) && (lock_detect_flag == 1) && (*perror_free_flag == 1))
			...
			if (abs(pvout_sampled[number_of_output_samples - 1] - pvin_sampled[number_of_input_samples - 1 - *pdelay]) != 0)
				{
				*pber = 1.0/number_of_error_free_samples;
				snprintf(plog_string,LOGFILE_LINELENGTH,
				"Detected an error...at time %s, BER = %1.4e based on %ld samples, re-synchronizing...\n",
				add_units(time,8,"s",value_string[0]),*pber,number_of_error_free_samples);
				print_string_to_log(plog_string,pdatain);
				*perror_free_flag = 0;
				number_of_error_free_samples = 0;
				*pdelay = -1;
				}
			else
				{
				number_of_error_free_samples++;
				*pber_sample_size = number_of_error_free_samples;
				*perror_free_flag = 1;
				*pber = 0.0;
				}
			}

	/* if (j == number_of_input_samples)
		*pdelay = -1; */
		}
	}
