...
double *pvco,int degree_of_polynomial,double vmax, double vmin,double errmax,int iter)
...
dvc1 = f1(pw,psig->vpd,psig->vc1,psig->vc2);
dvc2 = f2(pw,psig->vpd,psig->vc1,psig->vc2);
...
psig->vpd,psig->vc1,psig->vc2);*/
   k1 = h*f1(pw,psig->vpd,psig->vc1,psig->vc2);
   l1 = h*f2(pw,psig->vpd,psig->vc1,psig->vc2);
   k2 = h*f1(pw,psig->vpd,psig->vc1 + (b21*k1),psig->vc2 + (b21*l1));
   l2 = h*f2(pw,psig->vpd,psig->vc1 + (b21*k1),psig->vc2 + (b21*l1));
   k3 = h*f1(pw,psig->vpd,psig->vc1 + b31*k1 + b32*k2,
   psig->vc2 + b31*l1 + b32*l2);
   l3 = h*f2(pw,psig->vpd,psig->vc1 + b31*k1 + b32*k2,
   psig->vc2 + b31*l1 + b32*l2);
   k4 = h*f1(pw,psig->vpd,psig->vc1 + b41*k1 + b42*k2 + b43*k3,
   psig->vc2 + b41*l1 + b42*l2 + b43*l3);
   l4 = h*f2(pw,psig->vpd,psig->vc1 + b41*k1 + b42*k2 + b43*k3,
   psig->vc2 + b41*l1 + b42*l2 + b43*l3);
   k5 = h*f1(pw,psig->vpd,psig->vc1 + b51*k1 + b52*k2 + b53*k3 + b54*k4,
   psig->vc2 + b51*l1 + b52*l2 + b53*l3 + b54*l4);
   l5 = h*f2(pw,psig->vpd,psig->vc1 + b51*k1 + b52*k2 + b53*k3 + b54*k4,
   psig->vc2 + b51*l1 + b52*l2 + b53*l3 + b54*l4);
   k6 = h*f1(pw,psig->vpd,psig->vc1 + b61*k1 + b62*k2 + b63*k3 + b64*k4
   + b65*k5,psig->vc2 + b61*l1 + b62*l2 + b63*l3 + b64*l4 + b65*l5);
   l6 = h*f2(pw,psig->vpd,psig->vc1 + b61*k1 + b62*k2 + b63*k3 + b64*k4
   + b65*k5,psig->vc2 + b61*l1 + b62*l2 + b63*l3 + b64*l4 + b65*l5);
   /*printf("Value of freq = %6.4e\n",fo*(1 + deltaf(psig->vc2)));*/
   j1 = j2 = j3 = j4 = j5 = j6 = (h * 2.0 * pi * fo*(1 + deltaf_vco(psig->vc2,pvco,degree_of_polynomial)));
...
/*Compute next value and error*/
   ...
   signew.vpd = psig->vpd;
   /*printf("signew.vc1 = %2.2e signew.vc2 = %2.2e signew.phaseo = %2.2e"
   "signew.vpd = %2.2e\n",
   signew.vc1, signew.vc2, signew.phaseo, signew.vpd);*/
   ...
   if (reducestep == 1)
     ...
     /* printf(" to %2.4e seconds\n",h);*/
      ...
      /*Compute value of vc*/
      psig->vc = psig->vc1 + (pw->c7)*f1(pw,psig->vpd,psig->vc1,psig->vc2);
     /*printf("In runge-kutta, temp = %e, psig->vc1 = %e, f1 = %e\n",temp, psig->vc1,f1(pw,psig->vpd,psig->vc1,psig->vc2));*/
